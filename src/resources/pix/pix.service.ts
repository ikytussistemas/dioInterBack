...
export default class PixService {
  ...
  async request(value: number, user: Partial<User>) {
    ...
    const requestData = {
      requestingUser: currentUser, value, status: 'open',
    }
  ...
  async pay(key: string, user: Partial<User>){
    const keyDecoder = decodeKey(key);

    if(keyDecoder.userId === user.id){
    ...
    const requestingUser = await userRepository.findOne({where: {id: keyDecoder.userId}})
    const payingUser = await userRepository.findOne({where: {id: user.id}})

    if(payingUser?.wallet && payingUser.wallet < Number(keyDecoder.value)){
      throw new AppError('Não há saldo suficiente para fazer o pagamento', 401);
    }

    if(!requestingUser || !payingUser) {
      throw new AppError('Não econtramos os clientes da transação, gere uma nova chave', 401);
    }

    requestingUser.wallet = Number(requestingUser?.wallet) + Number(keyDecoder.value);
    await userRepository.save(requestingUser);
    
    payingUser.wallet = Number(payingUser?.wallet) - Number(keyDecoder.value);
    await userRepository.save(payingUser);
    
    const pixTransaction = await pixRepository.findOne({where: {id: keyDecoder.registerId, status: 'open'}})
